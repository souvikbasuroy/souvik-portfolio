      ...
      <div className="projects-grid">
        {PROJECTS.map((p) => (
          <div key={p.num} className="project-card glass" style={{
            ...
            <div className="project-img-wrap" style={{ height: '140px', width: '100%', overflow: 'hidden' }}>
              <img src={p.img} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>

            <div style={{ padding: '1.5rem' }}>
              <div className="project-num" style={{
                fontFamily: 'var(--font-heading)', fontSize: '1.2rem', color: 'var(--accent)', opacity: 0.3, fontWeight: 900
              }}>{p.num}</div>
              <h3 className="project-title" style={{ marginTop: '0.4rem', fontSize: '1.1rem', color: 'var(--text-bright)' }}>{p.title}</h3>
              <p className="project-desc" style={{ marginTop: '0.8rem', fontSize: '0.8rem', color: 'var(--text-muted)', minHeight: '3rem' }}>{p.desc}</p>

              <div className="project-tech" style={{ marginTop: '1rem', display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                {p.tech.map(t => (
                  <span key={t} className="tech-pill" style={{
                    fontSize: '0.5rem', padding: '3px 8px', background: 'rgba(255,215,0,0.05)',
                    border: '1px solid var(--accent)', color: 'var(--accent)', borderRadius: '4px', textTransform: 'uppercase'
                  }}>{t}</span>
                ))}
              </div>

              <div className="project-actions" style={{ marginTop: '1.5rem', display: 'flex', gap: '0.8rem' }}>
                <a href={p.github} target="_blank" rel="noopener noreferrer" className="glass" style={{
                  flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem',
                  padding: '8px', borderRadius: '6px', textDecoration: 'none', color: 'var(--text-bright)', fontSize: '0.65rem', fontWeight: 700
                }}>
                  <Github size={14} /> Code
                </a>
                <a href={p.live} target="_blank" rel="noopener noreferrer" style={{
                  flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem',
                  padding: '8px', borderRadius: '6px', textDecoration: 'none', background: 'var(--accent)', color: '#000', fontSize: '0.65rem', fontWeight: 700
                }}>
